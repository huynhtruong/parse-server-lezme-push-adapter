...
export default class MAPNS {
  ...
  constructor(args) {
    ...
    if (Array.isArray(args)) {
      ...
      throw new Parse.Error(Parse.Error.PUSH_MISCONFIGURED, 'MAPNS Configuration is invalid');
    }
  ...
  send(data, allDevices) {
    ...
    for (let key in devicesPerAppIdentifier) {
      ...
      let providers = this.mapProviders[key];
      ...
      let notification = MAPNS._generateNotification(coreData, headers);
  ...
  _handlePromise(response) {
    ...
    response.sent.forEach((token) => {
      log.verbose(LOG_PREFIX, 'MAPNS transmitted to %s', token.device);
  ...
  static _handlePushFailure(failure) {
    if (failure.error) {
      log.error(LOG_PREFIX, 'MAPNS error transmitting to device %s with error %s', failure.device, failure.error);
      ...
      log.error(LOG_PREFIX, 'MAPNS error transmitting to device %s with status %s and reason %s', failure.device, failure.status, failure.response.reason);
      return MAPNS._createErrorPromise(failure.device, failure.response.reason);
    } else {
       log.error(LOG_PREFIX, 'MAPNS error transmitting to device with unkown error');
