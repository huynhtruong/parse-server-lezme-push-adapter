...
var LOG_PREFIX = 'parse-server-push-adapter MAPNS';

var MAPNS = exports.MAPNS = function () {

  /**
   * Create a new provider for the APN service.
   * @constructor
   * @param {Object|Array} args An argument or a list of arguments to config MAPNS provider
  ...
  function MAPNS(args) {
    ...
    if (Array.isArray(args)) {
      ...
      throw new _parse2.default.Error(_parse2.default.Error.PUSH_MISCONFIGURED, 'MAPNS Configuration is invalid');
    }
    ...
    try {
      for (var _iterator = apnsArgsList[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        var provider = MAPNS._createProvider(apnsArgs);
  ...
  _createClass(MAPNS, [{
    ...
    value: function send(data, allDevices) {
      ...
      for (var key in devicesPerAppIdentifier) {
        ...
        var _appIdentifier = devices[0].appIdentifier;
        var providers = this.mapProviders[key];
        ...
        var headers = { expirationTime: expirationTime, topic: _appIdentifier, collapseId: collapseId };
        var notification = MAPNS._generateNotification(coreData, headers);
    ...
    value: function _handlePromise(response) {
      ...
      response.sent.forEach(function (token) {
        _npmlog2.default.verbose(LOG_PREFIX, 'MAPNS transmitted to %s', token.device);
        promises.push(MAPNS._createSuccesfullPromise(token.device));
      });
      response.failed.forEach(function (failure) {
        promises.push(MAPNS._handlePushFailure(failure));
      });
    ...
    value: function _handlePushFailure(failure) {
      if (failure.error) {
        _npmlog2.default.error(LOG_PREFIX, 'MAPNS error transmitting to device %s with error %s', failure.device, failure.error);
        return MAPNS._createErrorPromise(failure.device, failure.error);
      } else if (failure.status && failure.response && failure.response.reason) {
        _npmlog2.default.error(LOG_PREFIX, 'MAPNS error transmitting to device %s with status %s and reason %s', failure.device, failure.status, failure.response.reason);
        return MAPNS._createErrorPromise(failure.device, failure.response.reason);
      } else {
        _npmlog2.default.error(LOG_PREFIX, 'MAPNS error transmitting to device with unkown error');
        return MAPNS._createErrorPromise(failure.device, 'Unkown status');
      }
    }
