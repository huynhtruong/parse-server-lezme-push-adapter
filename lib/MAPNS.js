  ...
  _createClass(MAPNS, [{
      ...
      for (var key in devicesPerAppIdentifier) {
        ...
        if (!providers || providers.length === 0) {
          var errorPromises = devices.map(function (device) {
            return MAPNS._createErrorPromise(device.deviceToken, 'No Provider found');
          });
    ...
    value: function _createProvider(apnsArgs) {
      ...
      if (!MAPNS._validateAPNArgs(apnsArgs)) {
        throw new _parse2.default.Error(_parse2.default.Error.PUSH_MISCONFIGURED, 'topic is missing for %j', apnsArgs);
      }
